...
public abstract class DataBoundListAdapter<T, V extends ViewDataBinding>
    ...
    private static final String TAG = "DataBoundListAdapter";
    ...
    public final void onBindViewHolder(DataBoundViewHolder<V> holder, int position) {
        Log.i(TAG, "DataBoundListAdapter-onBindViewHolder: ");
    ...
    public void replace(final List<T> update) {
        dataVersion++;
        if (items == null) {
            Log.i(TAG, "DataBoundListAdapter-replace: items is null");
            if (update == null) {
                Log.i(TAG, "DataBoundListAdapter-replace: update is null");
                return;
            }
            items = update;
            notifyDataSetChanged();
        } else if (update == null) {
            Log.i(TAG, "DataBoundListAdapter-replace: update is null items is not null ");
            int oldSize = items.size();
            items = null;
            notifyItemRangeRemoved(0, oldSize);
        } else {
            Log.i(TAG, "DataBoundListAdapter-replace: all not null");
            ...
            new AsyncTask<Void, Void, DiffUtil.DiffResult>() {
                ...
                protected void onPostExecute(DiffUtil.DiffResult diffResult) {
                    Log.i(TAG, "DataBoundListAdapter-onPostExecute: update");
                    if (startVersion != dataVersion) {
                        // ignore update
                        Log.i(TAG, "DataBoundListAdapter-onPostExecute: ignore update");
                        return;
                    }
                    items = update;
