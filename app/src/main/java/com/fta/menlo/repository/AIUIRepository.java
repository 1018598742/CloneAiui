...
import com.fta.menlo.model.Settings;
import com.fta.menlo.ui.common.SingleLiveEvent;
...
import static com.fta.menlo.chat.RawMessage.FromType.AIUI;
import static com.fta.menlo.chat.RawMessage.FromType.USER;
import static com.fta.menlo.chat.RawMessage.MsgType.TEXT;
import static com.fta.menlo.chat.RawMessage.MsgType.Voice;
...
//    private AIUIPlayer mPlayer;
    ...
    public AIUIRepository(Context context, JSONObject config, String mscCfg, MessageDao dao, SettingsRepo settingsRepo) {
        mLastConfig = config;
        mMscCfg = mscCfg;
        mContext = context;
        mMessageDao = dao;
...
//        mPlayer = player;
        ...
        mAIUIListener = aiuiEvent -> {
            Log.i(TAG, "AIUIRepository-AIUIRepository: 类型：" + aiuiEvent.eventType);
    ...
    private void processResult(AIUIEvent event) {
        try {
            ...
            Log.i(TAG, "AIUIRepository-processResult: 获得的信息：" + event.info);
            ...
            if (content.has("cnt_id")) {
                ...
                Log.i(TAG, "AIUIRepository-processResult: 获得的详细信息：" + s);
                JSONObject cntJson = new JSONObject(s);
                String sub = params.optString("sub");
                Log.i(TAG, "AIUIRepository-processResult: sub 是：" + sub);
                if ("nlp".equals(sub)) {
                    ...
                    if (semanticResult != null && semanticResult.length() != 0) {
                        ...
                        if (semanticResult.has("answer")) {
                            JSONObject answer = semanticResult.optJSONObject("answer");
                            String text = answer.optString("text");
                            Log.i(TAG, "AIUIRepository-processResult: 答案：" + text);
                            TtsRepository.getInstance(mContext).playTts(text);
                        }else {
                            TtsRepository.getInstance(mContext).playTts("对不起，我不懂你的意思！");
                        }
